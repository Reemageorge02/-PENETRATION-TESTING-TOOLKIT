...
# ===================== MODULE 3: WHOIS LOOKUP (No external module) =====================
def whois_lookup():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[+] Registrar: {data.get('registrar', {}).get('name', 'N/A')}")
            print(f"[+] Status: {data.get('status', 'N/A')}")
            print(f"[+] Name Servers: {[ns['ldhName'] for ns in data.get('nameservers', [])]}")
            print(f"[+] Created: {data.get('events', [{}])[0].get('eventDate', 'N/A')}")
            print(f"[+] Updated: {data.get('events', [{}])[-1].get('eventDate', 'N/A')}")
        else:
            print(f"[!] Error: Unable to retrieve WHOIS info (status code {response.status_code})")
    except Exception as e:
